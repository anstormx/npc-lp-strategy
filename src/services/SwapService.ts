...
/**
 * Sleep utility to handle rate limiting
 */
...
export class SwapService {
  ...
  private nativeETHAddress: string = "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE";
  ...
  constructor(
    ...
    console.log(`
      --------------------------------
      Initialized SwapService with:
        Wallet address: ${this.walletAddress}
        Network: ${config.network} (Chain ID: ${config.chainId})
        1inch Router: ${ROUTER_ADDRESS}
      --------------------------------
    `);
  }
  ...
  private async executeApiRequest<T>(
    ...
    // Ensure at least 1100ms between requests to stay under rate limits
    const now = Date.now();
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (error.response?.status === 429 && attempt < maxRetries - 1) {
          ...
          const backoffTime = 1500 * Math.pow(2, attempt);
          console.log(`Rate limit exceeded (429). Retrying in ${backoffTime}ms. Attempt ${attempt + 1}/${maxRetries}`);
  ...
  private isNativeETH(tokenAddress: string): boolean {
    ...
    return tokenAddress.toLowerCase() === this.nativeETHAddress.toLowerCase();
  }

  /**
   * Approves token for spending by 1inch router
   */
  public async approveToken(tokenAddress: string, amount: BigNumber = ethers.constants.MaxUint256): Promise<void> {
    ...
    try {
      ...
      const response = await this.executeApiRequest<ApprovalResponse>(approvalUrl, {
  ...
  public async checkAllowance(tokenAddress: string): Promise<BigNumber> {
    ...
    try {
      ...
      const response = await this.executeApiRequest<AllowanceResponse>(allowanceUrl, {
        headers: { 'Authorization': `Bearer ${this.apiKey}`, 'accept': 'application/json' },
        params: {
          tokenAddress,
   ...
   */
  public async ensureAllowance(tokenAddress: string, amount: BigNumber): Promise<void> {
    ...
    try {
      ...
      if (allowance.lt(amount)) {
        console.log(`Insufficient allowance (${ethers.utils.formatEther(allowance)}), approving token...`);
        await this.approveToken(tokenAddress);
      } else {
        console.log(`Sufficient allowance already exists`);
      }
    } catch (error) {
      console.error(`Error ensuring allowance for ${tokenAddress}:`, error);
  ...
  public async swap(
    ...
    console.log(`Swapping tokens with 1inch: from ${fromTokenAddress} to ${toTokenAddress}`);
    console.log(`Amount: ${ethers.utils.formatEther(amount)}, Slippage: ${slippagePercent}%`);
    ...
    try {
      const srcToken = this.isNativeETH(fromTokenAddress) ? this.nativeETHAddress : fromTokenAddress;
      const dstToken = this.isNativeETH(toTokenAddress) ? this.nativeETHAddress : toTokenAddress;
      
      // Get swap transaction data from 1inch API
      const swapUrl = `${ONEINCH_API_URL}/${this.config.chainId}/swap`;
      const response = await this.executeApiRequest<SwapResponse>(swapUrl, {
        headers: { 'Authorization': `Bearer ${this.apiKey}`, 'accept': 'application/json' },
        params: {
          src: srcToken,
          dst: dstToken,
      ...
      if (swapData.dstAmount) {
        console.log(`Expected output amount: ${ethers.utils.formatEther(swapData.dstAmount)}`);
      }
      
      // Set value if sending ETH
      const value = this.isNativeETH(fromTokenAddress) ? amount : BigNumber.from(0);
      ...
      throw new Error(`Error swapping tokens: ${error}`);
    }
  }
}